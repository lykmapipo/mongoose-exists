...
const prepareQuery = (ids, schemaTypeOptions) => {
  ...
  // obtain ref model
  const Model = mongooseCommon.model(ref);
...
const normalizeOptions = (optns) => {
  ...
  if (_.isBoolean(optns)) {
    return common.mergeObjects(defaults, { exists: true });
  }
  ...
  if (_.isArray(optns)) {
    ...
    return common.mergeObjects(defaults, { exists, message });
  }

  // handle object option
  if (_.isPlainObject(optns)) {
    return common.mergeObjects(defaults, { exists: true }, optns);
  }
  ...
  return function existsValidator(value) {
    return new Promise(
      function checkIfExists(resolve, reject) {
        ...
        const ids = common.uniq(_.map([].concat(value), (id) => common.idOf(id) || id));
...
const existsPlugin = (schema /* , options */) => {
  ...
  mongooseCommon.eachPath(schema, (schemaPath, schemaType) => {
    // collect schemaType options
    const schemaTypeOptions = mongooseCommon.schemaTypeOptionOf(schemaType);
